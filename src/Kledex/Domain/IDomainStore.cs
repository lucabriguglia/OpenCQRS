...
namespace Kledex.Domain
{
    public interface IDomainStore
    {
        ...
        /// <returns></returns>
        Task SaveAsync<TAggregate>(IDomainCommand command, IEnumerable<IDomainEvent> events) 
        ...
        /// <param name="command">The command.</param>
        /// <param name="events">The events.</param>
        void Save<TAggregate>(IDomainCommand command, IEnumerable<IDomainEvent> events)
            where TAggregate : IAggregateRoot;
    ...
    public class DefaultDomainStore : IDomainStore
    {
        ...
        public void Save<TAggregate>(IDomainCommand command, IEnumerable<IDomainEvent> events) 
            where TAggregate : IAggregateRoot
        {
            throw new NotImplementedException(Consts.StoreRequiredMessage);
        }

        public Task SaveAsync<TAggregate>(IDomainCommand command, IEnumerable<IDomainEvent> events) 
            where TAggregate : IAggregateRoot
        {
            throw new NotImplementedException(Consts.StoreRequiredMessage);
        }
